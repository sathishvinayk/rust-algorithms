...
impl Shape {
    fn new(x: f64, y: f64) -> Self {
        ...
        let x_acce1 = 0.1 * legal_range.sample(&mut rng);
        let y_acce1 = 0.1 * legal_range.sample(&mut rng);
...
impl World {
    ...
    fn remove_shape(&mut self, n: usize) {
    ...
    fn update(&mut self) {
        ...
        if n > 0 {
            ...
            self.remove_shape(n.abs() as usize);
        }
...
fn main() {
    ...
    let mut window: PistonWindow = 
        ...
        .exit_on_esc(true)
        .build()
        .expect("Coult not create a window");
